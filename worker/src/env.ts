...
type WidenGeneratedString<T> = T extends string ? string : T;
type GeneratedEnv = {
  [Key in keyof Cloudflare.Env]?: WidenGeneratedString<Cloudflare.Env[Key]>;
};

export type Env = GeneratedEnv & {
  // Generated by `wrangler types`; kept required here because the app cannot
  // do useful work without its only durable binding.
  DB: D1Database;

  // Secrets are explicit because CI intentionally has no `.dev.vars`, so
  // Wrangler cannot discover them while generating Cloudflare.Env there.
  ...
  // Test/local-only seams are absent from production wrangler vars.
  ...
  if (authPasswordHash === "") {
    ...
    problems.push({
      ...
      detail:
        ...
        "a 16-byte salt, a 32-byte hash, and " +
